...
Once the installation is completed, you can start your Gatsby app to make sure everything went ok. Run the following command in your terminal.
...
# Converting Gatsby Config and Node API to TypeScript

In this part, we will examineto write `gatsby-config` in TypeScript.
...
Create the TypeScript version of the config file at the root of your directory `'gatsby-typescript-tutorial/'` and name it `gatsby-config.ts`:
